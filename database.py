...
class Database:
            ...
            with self.conn.cursor() as cursor:
                ...
                if user_exists:
                    ...
                    ''', (
                        ...
                        is_bot, language_code, chat_id,
                        contact, is_active, referral_code,
                        telegram_id
                    ))
                else:
                    ...
                    ''', (
                        ...
                        is_bot, language_code, chat_id, contact,
                        is_active, DEFAULT_REQUESTS, referral_code
                    ))
                
                self.conn.commit()
                logger.info(f"Успешно сохранен пользователь {telegram_id}")
                return True
    ...
    def get_user_by_referral_code(self, referral_code: str) -> Optional[int]:
        try:
            ...
            logger.error(f"Ошибка при выполнении get_user_by_referral_code: {e}")
            return None
